...
namespace ShadowProperties.Classes
{
    public class BogusOperations
    {
        ...
        /// <returns></returns>
        public static int PopulateContact1Table(int count)
        {
        ...
        public static List<Contact1> RemoveSomeRecords(int[] identifiers)
        {
            using var context = new Context();

            for (int index = 0; index < identifiers.Length; index++)
            {
                context.Remove(context.Contacts1.FirstOrDefault(x => x.ContactId == identifiers[index])!);
            }
